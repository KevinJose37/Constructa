...
class PurchaseOrderPaidInformation extends Component
{
	...
	private function userCanInteract(): bool
	{
		...
		if ($this->orderState->status === PurchaseOrderState::STATUS_PENDIENTE && $paid) {
			...
			// Para Contador
			if ($user->can('approved_account.purchase')) {
	...
	{
		if (!Auth::user()->can('approved_tech.purchase')) {
	...
	public function save(PurchaseOrderServices $purchaseService)
	{
		...
		$data = [
			...
			'approved_tech' => $this->form->approved_tech,
			'approved_account' => $this->form->approved_account,
		];
	...
	public function getCanShowTechDecisionProperty(): bool
	{
		return Auth::user()->can('approved_tech.purchase') &&
			$this->orderState->status !== PurchaseOrderState::STATUS_PAGADO && $this->orderState->status !== PurchaseOrderState::STATUS_POR_CONFIRMAR;
	}
	...
	{
		return Auth::user()->can('paid.purchase') &&
			$this->orderState->status === PurchaseOrderState::STATUS_POR_CONFIRMAR;
	}
