    ...
    <div class="card-body">
        <div class="table-responsive-sm mb-3">
            ...
            <div x-data="{ show: false }" x-show="show" x-init="
                Livewire.on('chapterSaved', () => {
                    show = true;
                    setTimeout(() => show = false, 3000);
                })
            " class="alert alert-success" style="display: none;" id="successMessage">
                    ...
                    <tbody>
                        @forelse ($chapters as $chapter)
                            <tr class="table-primary">
                                ...
                                <td class="text-end">
                                <button wire:click="deleteChapter({{ $chapter->id }})" type="button" class="btn btn-danger btn-sm">
    Eliminar Capítulo
</button>
                                </td>
                            </tr>
            ...
            <form wire:submit.prevent="saveChapter">
                <div class="modal-body">
                    <div class="mb-3">
                        ...
                        <input type="text" wire:model="chapter_number" class="form-control" placeholder="Ingrese el número de capítulo">
                    </div>
                    <div class="mb-3">
                        <label for="chapterName" class="form-label">Nombre del capítulo</label>
                        <input type="text" wire:model="chapter_name" class="form-control" placeholder="Ingrese el nombre del capítulo">
                    </div>
        ...
        <tbody>
            @foreach ($items as $index => $item)
                <tr wire:key="item-{{ $index }}">
                    <td>
                        <input type="text" wire:model="items.{{ $index }}.item_number" class="form-control" placeholder="Ej: 1.1">
                    </td>
                    <td>
                        <input type="text" wire:model="items.{{ $index }}.description" class="form-control" placeholder="Descripción">
                    </td>
                    <td>
                        <input type="number" step="0.01" wire:model="items.{{ $index }}.total" class="form-control" placeholder="Total">
                    </td>
                    <td>
...
<button type="button" wire:click.stop="addItem" wire:loading.attr="disabled" class="btn btn-info mt-2">
